...
class Network(object):
    ...
    def backprop(self, x, y):
        ...
        nabla_w are layer-by-layer lists of NumPy arryas, similar to self.biases and self.weights"""
        nabla_b = [np.zeros(b.shape) for b in self.biases]
        nabla_w = [np.zeros(w.shape) for w in self.weights]
        ...
        for b, w in zip(self.biases, self.weights):
            ...
            # Calculation activation in next layer
        ...
        # Note that the variable l in the loop below is used a little differently than the notation in Chapter 2 of
        # the book. Here, l = 1 means the last layer of neurons, l = 2 is the second-to-last layer, and so on. It's a
        # renumbering of the scheme in the book, used here to take advantage of the fact that Python can use negative
        # indices in lists
